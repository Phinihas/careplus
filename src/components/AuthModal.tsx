...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-emerald-100 dark:border-slate-800 overflow-hidden text-slate-800 dark:text-slate-100 transition-colors duration-300">
        ...
        <div className="p-6">
          {/* Tabs */}
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl mb-6 text-sm font-medium text-slate-600 dark:text-slate-300">
            <button
              onClick={() => { setMode('signin'); setLocalError(null); }}
              className={`flex-1 py-2 text-center rounded-lg transition-all cursor-pointer ${
                mode === 'signin' 
                  ? 'bg-white dark:bg-slate-700 text-emerald-800 dark:text-emerald-300 shadow-xs font-semibold' 
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-200 font-medium text-sm transition-all shadow-xs disabled:opacity-60 mb-4 cursor-pointer"
          >
          ...
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white dark:bg-slate-900 px-2 text-slate-400 font-medium">Or enter credentials</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-500/20"
                  />
                </div>
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full mt-2 py-2.5 px-4 bg-emerald-800 hover:bg-emerald-700 text-white font-semibold text-sm rounded-xl transition-all shadow-md shadow-emerald-900/20 disabled:opacity-50 cursor-pointer"
            >
          ...
          <div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                disabled={isSubmitting}
                className="flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-xl border border-emerald-200 dark:border-emerald-800/80 bg-emerald-50/60 dark:bg-emerald-950/40 hover:bg-emerald-100/60 dark:hover:bg-emerald-950/70 text-emerald-800 dark:text-emerald-300 text-xs font-medium transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                disabled={isSubmitting}
                className="flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-xl border border-amber-300 dark:border-amber-700/80 bg-amber-50/80 dark:bg-amber-950/40 hover:bg-amber-100/80 dark:hover:bg-amber-950/70 text-amber-950 dark:text-amber-300 text-xs font-semibold transition-colors cursor-pointer"
            ...
            <p className="text-[10px] text-slate-400 text-center mt-2">
              Admin console is strictly authorized for <strong className="text-slate-600 dark:text-slate-300">{AUTHORIZED_ADMIN_EMAIL}</strong>
            </p>
          </div>
        </div>
      </div>
