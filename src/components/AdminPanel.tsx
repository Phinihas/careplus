...
export const AdminPanel: React.FC<AdminPanelProps> = ({ departments }) => {
  ...
  if (!isAdmin) {
    return (
      <div className="max-w-3xl mx-auto my-16 p-8 bg-white rounded-3xl border border-rose-200 shadow-xl text-center">
        <div className="w-16 h-16 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center mx-auto mb-4">
          <Lock className="w-8 h-8" />
        </div>
        <h2 className="text-2xl font-bold text-slate-900 mb-2">Restricted Hospital Command Access</h2>
        <p className="text-sm text-slate-600 max-w-md mx-auto leading-relaxed mb-4">
          ...
          at <strong className="text-emerald-800 font-mono">{AUTHORIZED_ADMIN_EMAIL}</strong>.
        </p>
        <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl max-w-sm mx-auto text-xs text-slate-500 mb-6">
          <span>Current Account: </span>
          <strong className="text-slate-800">{user?.email || 'Not logged in'}</strong>
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-semibold uppercase tracking-wider flex items-center gap-1.5">
                ...
                Hospital Chief Administrator Console
              </span>
              <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-bold">
                NABH Certified Operations
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
              CarePulse Hospital Command Center
            </h1>
            <p className="text-slate-400 text-xs sm:text-sm mt-1">
              Authorized Administrator: <span className="text-emerald-400 font-semibold">{AUTHORIZED_ADMIN_EMAIL}</span> • Indian Standard Time (IST) Synchronized
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs transition-all shadow-md shadow-emerald-950/30 border border-emerald-500"
            >
      ...
      <div className="flex border-b border-slate-200 mb-6 overflow-x-auto">
        <button
          ...
          className={`py-3 px-5 text-sm font-semibold border-b-2 whitespace-nowrap transition-colors flex items-center gap-2 ${
            ...
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-1.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-600"
                />
              </div>
              ...
              <select
                ...
                className="py-1.5 px-3 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-700 focus:outline-none focus:border-emerald-600"
              >
                    ...
                    filteredAppointments.map((app) => (
                      <tr key={app.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          ...
                          <p className="text-[11px] text-slate-500">{app.patientEmail}</p>
                          <p className="text-[11px] text-emerald-800 font-medium">{app.patientPhone}</p>
                        </td>

                        <td className="py-3 px-4">
                          <p className="font-semibold text-slate-900">{app.doctorName}</p>
                          <p className="text-[11px] text-emerald-700">{app.doctorSpecialty}</p>
                          <p className="text-[11px] text-slate-400">{app.departmentName}</p>
                        </td>

                        <td className="py-3 px-4">
                          <p className="font-medium text-slate-900">{app.date}</p>
                          <p className="text-[11px] text-slate-500 font-semibold">{app.timeSlot}</p>
                        </td>

                        <td className="py-3 px-4 max-w-xs">
                          <p className="truncate font-medium text-slate-800">{app.reason}</p>
                          {app.symptoms && (
                            <p className="text-[11px] text-slate-400 truncate">{app.symptoms}</p>
                          )}
                          {app.prescription && (
                            <span className="inline-block mt-1 px-2 py-0.5 bg-emerald-50 text-emerald-800 rounded font-medium text-[10px] border border-emerald-200">
                              Rx Attached
                            </span>
                          )}
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            {app.status === 'pending' && (
                              <button
                                ...
                                className="py-1 px-2.5 rounded-lg bg-emerald-700 hover:bg-emerald-800 text-white font-semibold text-[11px] transition-colors shadow-xs"
                            ...
                            <button
                              ...
                              className="py-1 px-2 rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-100 text-[11px] font-medium transition-colors"
                            ...
                            {app.status !== 'completed' && app.status !== 'cancelled' && (
                              <button
                                ...
                                className="py-1 px-2 rounded-lg bg-teal-50 hover:bg-teal-100 text-teal-800 font-semibold text-[11px] transition-colors"
      ...
      {activeTab === 'doctors' && (
        <div className="space-y-6 animate-in fade-in">
          <div className="flex justify-between items-center bg-white p-4 rounded-3xl border border-slate-200 shadow-xs">
            <div>
              <h3 className="font-bold text-slate-900 text-sm">Empanelled Doctors & Surgeons</h3>
              <p className="text-xs text-slate-500">Manage clinical specialists, OPD consult fees (₹), clinic suites, and leave status</p>
            </div>
            <button
              onClick={() => openDoctorModal()}
              className="py-2 px-4 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-semibold shadow-xs flex items-center gap-1.5"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {doctors.map((doc) => (
              <div
                key={doc.id}
                className="bg-white rounded-3xl p-5 border border-slate-200 hover:border-emerald-300 transition-all shadow-xs flex flex-col justify-between"
              >
                <div>
                  ...
                  <p className="text-xs text-emerald-700 font-semibold">{doc.title}</p>
                  <p className="text-[11px] text-slate-500">{doc.departmentName}</p>
                  <p className="text-[11px] text-slate-400 mt-1">{doc.qualifications} • {doc.experienceYears} yrs experience</p>
      ...
      {activeTab === 'patients' && (
        <div className="space-y-6 animate-in fade-in">
          <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-xs flex justify-between items-center">
            <div>
              <h3 className="font-bold text-slate-900 text-sm">CarePulse Registered Patients (EHR)</h3>
              <p className="text-xs text-slate-500">Authenticated user medical identities, emergency contacts, and blood groups</p>
            </div>
              ...
              <table className="w-full text-left text-xs text-slate-600">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-700 uppercase font-semibold">
                  <tr>
                    ...
                    <th className="py-3.5 px-4 text-right">Consultations</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {patients.map((pat) => {
                    ...
                    return (
                      <tr key={pat.uid} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              <p className="font-bold text-slate-900">{pat.displayName}</p>
                        ...
                        <td className="py-3 px-4">
                          <span className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-800 font-bold text-[11px] border border-emerald-200">
                            {pat.bloodGroup || 'B+'}
                          </span>
                        </td>
      ...
      {activeTab === 'reports' && (
        <div className="space-y-6 animate-in fade-in">
          <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-xs flex justify-between items-center">
            <div>
              <h3 className="font-bold text-slate-900 text-sm">Hospital Pathology & Diagnostic Vault</h3>
              <p className="text-xs text-slate-500">Cross-department patient records, blood work, and imaging scans</p>
            </div>
            <span className="text-xs font-semibold text-slate-500">
            ...
            {reports.map((rep) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Patient: <strong className="text-slate-800">{rep.patientName}</strong></p>
                  <p className="text-[11px] text-slate-400 mt-1">{rep.labName}</p>

                  <p className="mt-3 p-2 bg-slate-50 rounded-lg text-xs text-slate-600 line-clamp-2">
                    {rep.summaryNotes}
                  </p>
                </div>
      ...
      {isDoctorModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/65 backdrop-blur-sm animate-in fade-in overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-2xl w-full overflow-hidden text-slate-800 my-8">
            ...
            <form onSubmit={handleSaveDoctor} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full py-2 px-3 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-emerald-600"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full py-2 px-3 text-xs rounded-xl border border-slate-200 bg-white focus:outline-none focus:border-emerald-600"
                  >
                    {departments.map((d) => (
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Clinic Room / OPD Location</label>
                <input
                  type="text"
              ...
              <div className="pt-3 border-t border-slate-100 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setIsDoctorModalOpen(false)}
                  className="py-2 px-4 rounded-xl border border-slate-200 text-xs font-medium text-slate-700 hover:bg-slate-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="py-2 px-5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-semibold text-xs shadow-md shadow-emerald-800/20"
                >
                  Save Doctor Profile
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/65 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-lg w-full overflow-hidden text-slate-800">
            ...
            <form onSubmit={handleSavePrescription} className="p-6 space-y-4">
              <div className="p-3 bg-emerald-50 rounded-2xl text-xs space-y-1 text-emerald-950 border border-emerald-200">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full py-2 px-3 text-xs rounded-xl border border-slate-200 font-mono focus:outline-none focus:border-emerald-600"
                />
              </div>
